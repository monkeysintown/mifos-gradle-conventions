...
|Status
|`proposed` or `rejected` or `accepted` or `deprecated` or `superseded by ADR-0123`
...
Chosen option: "title of option 1", because justification e.g., only option, which meets k.o. criterion decision driver | which resolves force xxx | … | comes out best (see below)}.

    ...
    public void apply(Settings settings) {
        settings.dependencyResolutionManagement(resolutionManagement -> {
            resolutionManagement.repositories(repositories -> {
                ...
                repositories.maven(repository -> {
                    repository.setUrl("https://mifos.jfrog.io");
                });
            });
        });
                ...
                if(projectIdPrefix.isPresent()) {
                    if(project.getName().startsWith(projectIdPrefix.get().toString())) {
                        ...
                        project.getExtensions().configure(PublishingExtension.class, publishing -> {
                            publishing.publications(publications -> publications.create("maven", MavenPublication.class, publication -> {
                                // project.getComponents().forEach(component -> log.error("Components: {}", component.getName()));
                                ...
                                publication.pom(pom -> {
                                    ...
                                    pom.developers(developers -> mifosDevelopers.forEach(mifosDeveloper -> developers.developer(dev -> {

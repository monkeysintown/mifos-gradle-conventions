...
import static org.mifos.convention.project.base.MifosProjectConstants.MIFOS_PROJECT_BOOT_MODULES;
import static org.mifos.convention.project.base.MifosProjectConstants.MIFOS_PROJECT_BOOT_MODULE_PATTERN;
...
public class MifosProjectBootLayoutPlugin implements Plugin<Settings> {
    private static final Logger log = Logging.getLogger(MifosProjectBootLayoutPlugin.class);
    ...
    public void apply(Settings settings) {
        ...
        for(int i=0; i < MIFOS_PROJECT_MAX_SIZE; i++) {
            ...
            if(modulesFolder.isPresent()) {
                ...
                try {
                    if(Files.exists(modulesBasePath)) {
                        Files.walkFileTree(modulesBasePath, EnumSet.noneOf(FileVisitOption.class), 3, new SimpleFileVisitor<>() {
                            ...
                            public @NonNull FileVisitResult visitFile(@NonNull Path file, @NonNull BasicFileAttributes attrs) throws IOException {
                                for(var module : MIFOS_PROJECT_BOOT_MODULES) {
                                    if(MIFOS_PROJECT_BOOT_MODULE_PATTERN.matcher(file.toString()).matches()) {
                                        var result = include(settings, projectIdPrefix, file.getParent().getParent().getFileName() + "-", module,  file.getParent().getParent().getFileName() + "/" + module + "/build.gradle", file);
                                        ...
                                        if(result.isPresent()) {
                                            ...
                                            projectPluginIds.put(result.get(), "org.mifos.convention.project.boot.%s.%s".formatted(projectClassifier, module));
                    ...
                    log.error("Error traversing boot directory: ", e);
                }
            } else {
                break;
            }
                ...
                settings.getGradle().beforeProject(project -> {
                    if(projectPluginIds.containsKey(project.getName())) {
                        ...
                        if(!project.getName().endsWith("core")) {
                            var coreModule = Path.of(project.getBuildFile().getParent()).getParent().getParent().resolve("core/build.gradle");

                            if(Files.exists(coreModule)) {
                                var coreProjectDep = project.getDependencies().project(Map.of("path", ":" + projectIdPrefix  + project.getBuildFile().toPath().getParent().getParent().getParent().getFileName() + MIFOS_PROJECT_SUFFIX_CORE));
        ...
        settings.getPlugins().apply(MifosProjectDocLayoutPlugin.class);
        settings.getPlugins().apply(MifosProjectBootSupportLayoutPlugin.class);
        settings.getPlugins().apply(MifosProjectBootSdkLayoutPlugin.class);
        settings.getPlugins().apply(MifosProjectBootPersistenceLayoutPlugin.class);
        settings.getPlugins().apply(MifosProjectBootUseCaseLayoutPlugin.class);
        settings.getPlugins().apply(MifosProjectBootServiceLayoutPlugin.class);
        settings.getPlugins().apply(MifosProjectBootFlowLayoutPlugin.class);
        settings.getPlugins().apply(MifosProjectBootTransportLayoutPlugin.class);
        settings.getPlugins().apply(MifosProjectBootStarterLayoutPlugin.class);
        settings.getPlugins().apply(MifosProjectBootApplicationLayoutPlugin.class);

        // TODO: operation, command, flow, integration, dataframe, client
    }
}

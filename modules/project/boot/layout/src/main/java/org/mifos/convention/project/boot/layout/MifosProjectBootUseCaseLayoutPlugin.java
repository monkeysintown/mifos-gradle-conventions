                            ...
                            public @NonNull FileVisitResult visitFile(@NotNull Path file, @NonNull BasicFileAttributes attrs) throws IOException {
                                ...
                                for(var module : MIFOS_PROJECT_BOOT_USECASE_MODULES) {
                                    if(MIFOS_PROJECT_BOOT_MODULE_USECASE_PATTERN.matcher(file.toString()).matches()) {
                                        var result = include(settings, projectIdPrefix, file.getParent().getParent().getParent().getParent().getFileName() + "-usecase-" + file.getParent().getParent().getFileName(), "-" + module, "usecase/" + file.getParent().getParent().getFileName() + "/" + module + "/build.gradle", file);
                                ...
                                if(projectPluginIds.containsKey(project.getName())) {
                                    ...
                                    if(project.getName().endsWith(MIFOS_PROJECT_SUFFIX_MAPPING)) {
                                        ...
                                        if(Files.exists(coreModule)) {
                                            var coreProjectDep = project.getDependencies().project(Map.of("path", ":" + projectIdPrefix  + rootModule.getFileName() + "-usecase-" + parentModule.getFileName() + MIFOS_PROJECT_SUFFIX_CORE));
                                    ...
                                    if(project.getName().endsWith(MIFOS_PROJECT_SUFFIX_IMPLEMENTATION)) {
                                        ...
                                        if(Files.exists(mappingModule)) {
                                            var mappingProjectDep = project.getDependencies().project(Map.of("path", ":" + projectIdPrefix  + rootModule.getFileName() + "-usecase-" + parentModule.getFileName() + MIFOS_PROJECT_SUFFIX_MAPPING));
